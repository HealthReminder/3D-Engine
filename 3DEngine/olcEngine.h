...
public:
	...
	{	
		// Start the thread
		m_bAtomActive = true;
		std::thread t = std::thread(&olcEngine::GameThread, this);
...
protected: // Audio Engine =====================================================================
		...
		unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		m_AudioThread = std::thread(&olcEngine::AudioThread, this);
	...
	{
		((olcEngine*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
...
std::atomic<bool> olcEngine::m_bAtomActive(false);
std::condition_variable olcEngine::m_cvGameFinished;
std::mutex olcEngine::m_muxGame;
